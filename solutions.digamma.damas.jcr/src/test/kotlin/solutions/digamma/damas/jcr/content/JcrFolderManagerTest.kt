...
class JcrFolderManagerTest : WeldTest() {
    ...
    private var manager: FolderManager? = null
    private var token: Token? = null

    @Before
    @Throws(Exception::class)
    fun setUp() {
        this.manager = WeldTest.inject(JcrFolderManager::class.java)
    ...
    fun updateName() {
        ...
        assert(rootId == folder.parentId) { "Updated folder parent ID mismatch." }
    }
    ...
    fun updateParent() {
        ...
        assert(id == folder.id) { "Updated folder ID mismatch." }
        assert(name == folder.name) { "Updated folder name mismatch." }
        assert(parentId == folder.parentId) { "Updated folder parent ID mismatch." }
    }
