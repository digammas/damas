...
class Lexer(
    ...
    companion object {
        const val BEGIN_NAMESPACE = '<'
        const val END_NAMESPACE = '>'
        const val NAMESPACE_URI = '='
    ...
    /**
     * Creates a failure exception including the current line number.
     ...
     */
    @Throws(ParseException::class)
    fun fail(message: String, e: Exception): Nothing {
        throw ParseException(message, e, lineNumber, -1)
    }

    /**
     * Creates a lexer on top of the given reader.
     * @param r the reader
     */
    init {
